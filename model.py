...
def overlaps_graph(boxes1, boxes2):
    ...
    '''
    #4. compute IoG(intersecton over graound truth)
    iog = intersection / b2_area
    overlaps = tf.reshape(iog, [tf.shape(boxes1)[0], tf.shape(boxes2)[0]])

    '''
    '''
    f = tf.get_default_session()
    print("it is here")
    boxes2 = tf.Print(boxes2, [boxes2], message="this is gt box")
    b = tf.add(boxes2, boxes2)
    b.eval(session=f)
    '''
    return overlaps
...
def detection_targets_graph(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    #Sara: not all of the proposals are ROI. So, here I think it extracts the ROIs from the proposals
    ...
    positive_roi_bool = (roi_iou_max >= 0.5)#Sara changed 0.5 to 0.3
    positive_indices = tf.where(positive_roi_bool)[:, 0]
    # 2. Negative ROIs are those with < 0.5 with every GT box
    negative_indices = tf.where(roi_iou_max < 0.5)[:, 0]#Sara changed 0.5 to 0.3
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    ...
    #Sara: input_feature_map = <tf.Tensor 'input_rpn_feature_map:0' shape=(?, ?, ?, depth) dtype=float32>
...
############################################################
#Sara: fpn=feature pyramid network
...
def mrcnn_mask_loss_graph(target_masks, target_class_ids, pred_masks):
    ...
    loss = K.switch(tf.size(y_true) > 0,
                    ...
                    tf.constant(0.0))
    loss = K.mean(loss)
    loss = tf.scalar_mul(0, loss) # Sara added this to avoid mask loss
    loss = K.reshape(loss, [1, 1])
    return loss
...
def load_image_gt(dataset, config, image_id, augment=False,
    ...
    image = dataset.load_image(image_id)#this loads the acctual image with no resizing
    mask, class_ids = dataset.load_mask(image_id) # the information that have been saved for maskes are after resizing
    mask_temp, class_ids_temp = mask, class_ids
    ...
    shape = image.shape
    #mask = utils.resize_mask(mask, scale, padding) ##Sara commented this out because the mask are made when the image had the resized size. Thus no need to resize the mask again
    ...
    bbox = utils.extract_bboxes(mask, dataset, image_id)
...
class MaskRCNN():
    ...
    def build(self, mode, config):
        ...
        #_, C2, C3, C4, C5 =  build_vgg_graph(input_image, stage5=True)
        # Top-down Layers
        #Sara: feature pyrimid network is a top-down architecture
        ...
        plot_model(model, to_file = 'modelvgg.png') 
        return model
    ...
    def compile(self, learning_rate, momentum):
        ...
        """
        # Optimizer object
        #Sara Commented this out and replaced it with Adam
        optimizer = keras.optimizers.SGD(lr=learning_rate, momentum=momentum, clipnorm=5.0)
        #optimizer = keras.optimizers.Adam(lr=learning_rate, beta_1=0.9, beta_2=0.999, epsilon=None, decay=0.0, amsgrad=False)
        '''
        ##Sara: Found the following lines from internet(for having decay)
        ...
        for name in loss_names:
            layer = self.keras_model.get_layer(name)#Sara: this gives name "name" to the layer
            if layer.output in self.keras_model.losses:
                continue
            self.keras_model.add_loss(
                    tf.reduce_mean(layer.output, keep_dims=True))#Sara: adds a externally defined loss to the collection of losses
            #Sara: tf.reduce_mean reduces input_tensor along the dimensions given in axis. Unless keepdims is true, the rank of the tensor is reduced by 1 for each entry in axis. If keepdims is true, the reduced dimensions are retained with length 
        ...
        #Sara: tf.add_n(reg_lossess) adds all reg_losses tensors element-wise.

        # Compile
        self.keras_model.compile(optimizer=optimizer, loss=[
                                 None] * len(self.keras_model.outputs))
        #Sara:* because we have different output and the loss is calculated base on all of them
    ...
    def train(self, train_dataset, val_dataset, learning_rate, epochs, layers):
        ...
        callbacks = [
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path, monitor='val_loss', verbose=0, mode='min',
	...
	# Callbacks
        callbacks = [ 
            ...
            keras.callbacks.ModelCheckpoint(self.checkpoint_path, monitor='val_loss', verbose=0, mode='min', save_best_only=True, save_weights_only=True)
        ]
        ...
        #print(self.config.VALIDATION_STEPS)
    ...
    def mold_inputs(self, images):
        ...
        for image in images:
            ...
            # Build image_meta
            #in the following it was image Sara changed it to molded_image
    ...
    def detect(self, images, verbose=0):
        ...
        # Process detections
        results = []
        images = molded_images #sara added this
